  ...
  module ShutdownHelper
    ...
    def self.intercom_shutdown_helper(cookies, domain)
      if (cookies.is_a?(ActionDispatch::Cookies::CookieJar))
        cookies["intercom-session-#{IntercomRails.config.app_id}"] = { value: nil, expires: 1.day.ago, domain: domain}
      else
        controller = cookies
        ...
        controller.response.delete_cookie("intercom-session-#{IntercomRails.config.app_id}", { value: nil, expires: 1.day.ago}, domain: domain)
      end
    rescue
    end
